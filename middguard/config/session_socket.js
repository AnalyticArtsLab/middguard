// session.socket.io (MIT License)
// https://github.com/eiriklv/session.socket.io
...
module.exports = function(io, sessionStore, cookieParser, key) {
  ...
  function findCookie(handshakeInput) {
    ...
    if (handshake.secureCookies && handshake.secureCookies[key])
      handshake.secureCookies = (handshake.secureCookies[key].match(/\:(.*)\./) || []).pop();
    if (handshake.signedCookies && handshake.signedCookies[key])
      handshake.signedCookies[key] = (handshake.signedCookies[key].match(/\:(.*)\./) || []).pop();
    if (handshake.cookies && handshake.cookies[key])
      handshake.cookies[key] = (handshake.cookies[key].match(/\:(.*)\./) || []).pop();
