  ...
  var Messages = Backbone.Collection.extend({
    ...
    messagesPerUser: function() {
      ...
      this.each(function(message) {
        ...
        if (nativeHasOwnProperty.call(stats, analyst)) {
          stats[analyst]++;
        } else {
          stats[analyst] = 1;
        }
    ...
    messagesInInterval: function(buckets) {
      ...
      // i.e. show how many messages were sent in each
      // time interval for some number of time intervals
      var stats = {},
          minTS, maxTS,
          TSList = [];

      this.each(function(message) {
        // run through once to find the minimum timestamp,
        // maximum timestamp, and range
        ...
        if (!minTS && !maxTS) {
          ...
          if (timestamp < minTS) {
            ...
            // as long as minTS and maxTS are defined,
            // a new timestamp cannot be both the minTS and maxTS
            // thus, we use "else if"
            maxTS = timestamp;
          }
        }
      ...
      TSList.forEach(function(curTS) {
        ...
        var size = Math.floor((curTS.getTime() - minTS.getTime()) / interval),
            bucket = Math.min(4, size);
